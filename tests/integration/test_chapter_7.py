...
from htma_py.continuous_evpi import calculate_evpi, get_lin_payoff_and_loss
...
def test_chapter_7_evpi():
    ...
    evpi_exact_gauss = calculate_evpi(
        payoff_gauss_dist, lin_payoff_array, lin_loss_array
    )

    # NOTE: We need a lot of samples in order for the test to pass
    units_data_dist = DistributionFromSamples(
        payoff_gauss_dist.sample(int(1e5)), min_x_value=units_min * price_per_unit
    )
    evpi_data_from_distribution = calculate_evpi(
        units_data_dist, lin_payoff_array, lin_loss_array
    )
